...
The application provides both a Command Line Interface (CLI) for input of data and a Graphical User Interface (GUI) for displaying of patient's personal details and health-related information.
...
Currently, most health care professionals rely on paper documents that contains such data for each patients. Not only do they have to perform the menial task of searching through the piles of patient documents, sometimes, they have to look through the entire patient document to get what they need to know about the patient.

Health Book is aimed at solving such administrative woes of health care professionals. By digitizing such data into an application, health care professionals can easily retrieve the patient data that they need, as well as the specific information that they need for each patient.
...
|List Patient |Shows the list of all patient profile
|Select Patient | Select a patient to view its entire profile
|Add Additional Information| Add or edit additional information such as patient's bio-data
...
|Find/Sort | Manipulate how patients are presented with find that filters the patient or sort which arranges the patients based on the parameters used
