...
The purpose of this portfolio is to document my contributions towards the Health Book. It showcases the implementation of my features, the codes I have written and the changes I have made to the User and Developer Guides.

== Overview

Health Book is an application written in Java catered to healthcare professionals in retrieving patients’ personal information and health-related information all in one place. Command Line Interface (CLI) is utilized mainly for user input, while Graphical User Interface (GUI) is used mainly to display the output. Combining these 2 interfaces, Health Book aims to provide healthcare professionals with the speed and efficiency obtained from a CLI while retaining the systematic view of information through the GUI.
...
** Credits: The predicates are lightly adapted from the NameContainsKeywordsPredicate
...
*** Created a java class for Blood Type: https://github.com/CS2113-AY1819S1-T12-2/main/blob/master/src/main/java/seedu/address/model/person/addinfo/BloodType.java[Blood Type class]
** Documentation:
