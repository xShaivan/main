...
Adds an appointment for the 2nd person in the list on 16-09-2018 from 15:00-15:30 at Consultation Room 12
for a diabetes checkup by Dr Tan.

=== Viewing a patient's appointments timetable: `viewappt`

Shows details of a specified appointment for the specified patient. +
Format: `viewappt INDEX s/START`

****
* Views an appointment with the specified start time for the patient at the specified index.
* The index refers to the index number shown in the displayed person list.
****

Examples:

* `viewappt 2 s/16-09-2018 15:00` +
Views the appointment that starts on 16-09-2018 at 15:00 for the 2nd person in the list.
...
Format: `editappt INDEX os/ORIGINAL_START [d/DATE] [s/START_TIME] [e/END_TIME]
...
* NRIC input must be valid with accordance to ICA's NRIC checksum
...
* Decimals is not supported as of this release.
...
* Alternative input `Male` and `Female` are acceptable also.

.Constraints for `[o/OCCUPATION]`
* Must not contain any numerics and special characters including whitespaces (Eg. Nurse and Doctor or Wood-Logger are not accepted input).
...
Format: `editreport INDEX d/DATE [t/TITLE] [d/DATE] [i/INFORMATION]`

Examples:

* `editreport 3 d/01-01-2018 t/Depression d/02-02-2018 i/Prescribed XXX medicine, next appointment on 03-03-2018.` +
Edits title, date and information of existing medical report dated 01-01-2018 for patient at index 3.

=== Viewing of Medical Report: `viewreport`

Views the medical report of the patient on a certain date. +
Format: `viewreport INDEX d/DATE`

Examples:

* `viewreport 1 d/01-01-2018` +
View medical report dated 01-01-2018 for patient at index 1.
// end::reportcommands[]

=== Sort patients list: `sort`

Sort the list of patients according to a specific prefix category. +
Format: `sort PREFIX/ ORDER_INDEX`

`PREFIX/` refers to certain categories of patient information such as name, blood type,
allergy etc. and each of these categories will be defined by a corresponding prefix. (ie n/ for name)
...
* *View Appt* : `viewappt INDEX s/START` +
e.g. `viewappt 2 s/16-09-2018 15:00`
