...
Figure 4.3.1: Sequence Diagram for `addhist` command
...
Figure 4.4.1: Sequence Diagram for `edithist` command
...
Figure 4.6.1 shows how the addreport operation works:

image::AddMedicalReportSequenceDiagram.png[width="800"]
Figure 4.6.1: Sequence Diagram of AddMedicalReport feature
// end::addreport[]
...
The find mechanism is facilitated by `ModelManager`. It extends `AddressBook` and finds the `internalList` of patients. Additionally,
...
IMPORTANT: The list that the doctor will be implementing the `find` command on will the be full patient list and details stored in the database.
...
WARNING: If the command format is invalid, an error message will be shown to prompt the user of the correct format.
...
NOTE: There must be more than one keyword and the prefix is case sensitive.
...
Step 1. A doctor launches the application for the first time. If it hs/her first time launching the Health Book, the `VersionedAddressBook` will be initialized with a sample address book data. If he/she has launched it and has made changes before, the `VersionedAddressBook` will be launched with the data he has previously saved in his previous launch.
IMPORTANT: The list that the doctor will be implementing the sort command on will the be current patient list as shown in the interface.
...
|`* * *` |doctor |filter out patients by certain personal details |save time in contacting patients with desired personal details

|`* * *` |doctor |filter out patients by certain medical details |save time in administering patients with desired medical details

|`* * *` |doctor |sort patients by patient details by lexicographical or reverse order |go through the patient list which is listed in a systematic manner to facilitate administration

|`* * *` |doctor |sort all my patients by their personal particulars |ensure systematic administration and processing of each individual
...
Assume that user has already requested the System to display a list of patients.
NOTE: This list refers to the latest list displayed on the interface before the command execution.
